        ...
        <>
            <div className='bg-white'>
                ...
                <div className="w-screen h-screen grid grid-cols-12 bg-white container mx-auto">
                    ...
                    {/* 11 column area for shades */}
                    <div className="col-span-10 p-6 grid grid-cols-12 transition-all">
                        {selectedColor &&
                            colorToClassMap[selectedColor].map((className, index) => {
                                const totalShades = colorToClassMap[selectedColor].length;
                                const colSpan = Math.floor(12 / totalShades);

                                return (
                                    <div
                                        key={`${selectedColor}-${index}`} // important to trigger re-render
                                        className={`cursor-pointer w-full ${className} col-span-${colSpan} pop-in`}
                                        title={className}
                                        style={{ animationDelay: `${index * 50}ms` }} // stagger delay
                                        onClick={handleCopy}
                                    />
                                );
                            })}
                    ...
                    {/* 1 column area for color selection buttons */}
                    <div className="h-100 overflow-auto col-span-2 flex flex-col m-6 mr-0 pr-6 custom-scroll">
                        {Object.keys(colorToClassMap).map((color) => (
                            <button
                                key={color}
                                className={`w-full p-2 mb-4 rounded text-white capitalize ${colorToClassMap[color][4]}`}
